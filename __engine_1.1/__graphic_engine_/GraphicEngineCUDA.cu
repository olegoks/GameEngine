...
{
	FramePixel rgb_pixel;
	rgb_pixel.rgba_blue = rgb_color.rgba_blue;
	rgb_pixel.rgba_green = rgb_color.rgba_green;
	rgb_pixel.rgba_red = rgb_color.rgba_red;
	rgb_pixel.rgba_alpha = rgb_color.rgba_alpha;

	const unsigned int number_of_threads = 1024;

	const unsigned int number_of_blocks = (1920 * 1080 * sizeof(FramePixel) + number_of_threads - 1) / number_of_threads;
...
FrameHost MeshFrame(DeviceData* device_data, const Camera* camera)noexcept(true) {
	...
	/////*FrameHost frame{ 1920, 1080 };
	////
	////FramePixel color{ RandomColor() };
	...
	static const Vertex2D* const device_vertexs_2d = GpuMemoryManager::Allocate<Vertex2D>(device_data->PrimitivesNumber().n_vertexs);

	const Vertex3D& camera_position = camera->GetPosition();;
	//
	const Vector3D& vector_x = camera->GetVectorX();
	const Vector3D& vector_y = camera->GetVectorY();
	const Vector3D& vector_z = camera->GetDirection();

	const float proj_plane_dist = camera->GetDistanceToProjPlane();

	DevicePtrs ptrs;
	ptrs = device_data->Ptrs();

	NPrimitives n_primititives;
	n_primititives = device_data->PrimitivesNumber();
	...
	const unsigned int number_of_threads = 1024;
	unsigned int number_of_blocks = (n_primititives.n_vertexs + number_of_threads - 1) / number_of_threads;

	//SetDisplayBufferColor(RgbColor{ 20, 255, 0 });

	//ProjectVertexs <<<number_of_blocks, number_of_threads>>> (ptrs.vertexs, device_vertexs_2d, n_primititives.n_vertexs, camera_position, vector_x, vector_y, vector_z, distance_to_projection_plane);
...
//FrameHost RenderMeshFrame()noexcept(true) {
	...
	//static const Vertex2D* vertexs_2d = GpuMemoryManager::Allocate<Vertex2D>();
	...
	//const Vertex3D* const device_vertexs_3d = device_data_.Ptr<Vertex3D>();
